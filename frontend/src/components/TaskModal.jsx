      ...
      <div className={`relative glass-panel rounded-2xl p-8 w-full max-w-lg shadow-2xl transition-all duration-500 transform ${open ? 'translate-y-0 scale-100 opacity-100' : 'translate-y-10 scale-95 opacity-0'}`}>
        <header className="flex items-center gap-3 mb-6">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary to-primary-container text-on-primary flex items-center justify-center shadow-lg">
            <span className="material-symbols-outlined text-[28px]">{initial ? "edit_note" : "add_task"}</span>
          </div>
          <div>
            ...
            <p className="text-on-surface-variant text-sm font-medium">{initial ? "Update task details below." : "Fill in the details to create a new task."}</p>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="bg-white/40 border border-white/60 rounded-2xl p-5 mt-2 shadow-inner">
            ...
            <div className="flex flex-col gap-1.5 group">
              <label className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider ml-1">Due Date</label>
              <div className="relative datepicker-wrapper w-full">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-primary pointer-events-none z-10 text-[18px]">calendar_today</span>
                <DatePicker
                  selected={form.dueDate ? new Date(form.dueDate) : null}
                  onChange={(date) => setForm({ ...form, dueDate: date ? date.toISOString().slice(0, 10) : "" })}
                  className="w-full pl-10 pr-4 py-3 bg-white border border-white/80 rounded-xl focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all shadow-sm font-medium text-on-surface cursor-pointer"
